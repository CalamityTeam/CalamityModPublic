    ...
    public class CalamityPlayer : ModPlayer
    {
        ...
        public override bool Shoot(Item item, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            if (veneratedLocket)
            {
                if (item.Calamity().rogue)
                {
					Projectile proj = CalamityUtils.ProjectileToMouse(player, 1, item.shootSpeed, 0f, 15f, type, (int)(damage * 0.15f), knockBack * 0.5f, player.whoAmI, true);
                    proj.Calamity().forceRogue = true; //in case melee/rogue variants bug out
