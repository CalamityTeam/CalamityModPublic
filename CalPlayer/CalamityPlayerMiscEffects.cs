...
namespace CalamityMod.CalPlayer
{
	public class CalamityPlayerMiscEffects
	{
		...
		private static void MiscEffects(Player player, CalamityPlayer modPlayer, Mod mod)
		{
			...
			bool canProvideBuffs = player.Calamity().profanedCrystalBuffs || (!modPlayer.profanedCrystal && modPlayer.pArtifact) || (player.Calamity().profanedCrystal && CalamityWorld.downedSCal);
			...
			if (!modPlayer.bOut && canProvideBuffs)
			{
				...
				if (healer)
				{
					...
					if (modPlayer.healCounter <= 0)
					{
						...
						modPlayer.healCounter = (!enrage && player.Calamity().profanedCrystalBuffs) ? 360 : 300;
						if (player.whoAmI == Main.myPlayer)
						{
							int healAmount = 5 +
								(defend ? 5 : 0) +
		...
		private static void OtherBuffEffects(Player player, CalamityPlayer modPlayer)
		{
				...
				if (player.statLife > (int)(player.statLifeMax2 * 0.25))
				{
					...
					if (player.whoAmI != Main.myPlayer && player.miscCounter % 10 == 0)
					{
						...
						if (Main.player[myPlayer].team == player.team && player.team != 0)
						{
							...
							if ((float)Math.Sqrt(arg * arg + num3 * num3) < 800f)
			...
			if (modPlayer.prismaticLasers == 1)
			{
                ...
                for (int dustIndex = 0; dustIndex < dustAmt; dustIndex++)
                {
                    ...
                    Vector2 vector6 = Vector2.Normalize(player.velocity) * new Vector2(player.width / 2f, player.height) * 0.75f;
                    vector6 = vector6.RotatedBy((dustIndex - (dustAmt / 2 - 1)) * MathHelper.TwoPi / dustAmt, default) + player.Center;
