...
namespace CalamityMod
{
	public static class CalamityUtils
	{
		...
		public static void PickAmmoSummon(this Player player, Item item, ref int shoot, ref float speed, ref bool canShoot, ref int dmg, ref float kBack, bool dontConsume = false)
		{
			Item ammo = null;
			bool ammoSlots = false;
			// Check if there is ammo in the dedicated ammo slots first
			...
			if (ammo is null || !canShoot)
				return;

			if (ammo.shoot > 0)
				shoot = ammo.shoot;
			speed += ammo.shootSpeed;
			kBack += ammo.knockBack;

			// Main.NewText("Damage before: " + dmg, Color.Green);
			int ammoBonus = 0;
			if (ammo.damage > 0)
			{
				ammoBonus = (int)(ammo.damage * player.MinionDamage());
				// Main.NewText("Ammo Bonus: " + ammoBonus, Color.Red);
			}
			dmg += ammoBonus;
			// Main.NewText("Damage after: " + dmg, Color.Red);
			...
			if (Main.rand.NextFloat() > player.Calamity().rangedAmmoCost)
				dontConsumeAmmo = true;
			if (dontConsumeAmmo || !ammo.consumable)
				return;
			--ammo.stack;
			if (ammo.stack > 0)
				return;
