        ...
        public override void AI()
        {
			...
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				if (!TailSpawned)
				{
					...
					{
						int num4 = ModContent.NPCType<DriedSeekerBody>();
						if (j == num3)
						{
							num4 = ModContent.NPCType<DriedSeekerTail>();
						}
			...
			if (Main.player[npc.target].dead)
			{
				...
				if ((double)npc.position.Y > Main.rockLayer * 16.0)
				{
					for (int a = 0; a < 200; a++)
					{
						if (Main.npc[a].type == ModContent.NPCType<DriedSeekerHead>() || Main.npc[a].type == ModContent.NPCType<DriedSeekerBody>() ||
							Main.npc[a].type == ModContent.NPCType<DriedSeekerTail>())
						{
