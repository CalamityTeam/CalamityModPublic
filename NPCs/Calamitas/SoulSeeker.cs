    ...
    public class SoulSeeker : ModNPC
    {
        ...
        public override bool PreAI()
        {
            ...
            if (timer >= 180)
            {
                if (Main.netMode != NetmodeID.MultiplayerClient && parent.ai[1] < 2f && parent.Calamity().newAI[2] <= 0f)
                {
                    ...
                    Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, velocity, type, damage, 1f, NPC.target, 1f, 0f);
                }
                timer = 0;
            }
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            ...
            if (NPC.life <= 0)
            {
                ...
                {
                    Gore.NewGore(NPC.position, NPC.velocity, Mod.Find<ModGore>("SoulSeeker").Type, 1f);
                    Gore.NewGore(NPC.position, NPC.velocity, Mod.Find<ModGore>("SoulSeeker2").Type, 1f);
                    Gore.NewGore(NPC.position, NPC.velocity, Mod.Find<ModGore>("SoulSeeker3").Type, 1f);
                }
