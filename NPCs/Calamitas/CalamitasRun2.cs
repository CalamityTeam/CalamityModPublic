...
namespace CalamityMod.NPCs.Calamitas
{
	...
	public class CalamitasRun2 : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			animationType = 126;
			npc.alpha = 50;
			npc.lifeMax = CalamityWorld.revenge ? 3375 : 2500;
            ...
            {
                npc.lifeMax = CalamityWorld.death ? 1200000 : 900000;
            }
        }
		...
		public override void AI()
		{
			...
			else
			{
				...
				if (npc.ai[1] == 2f)
				{
					...
					if (npc.life <= (npc.lifeMax * 0.75f) || CalamityWorld.bossRushActive)
					{
						npc.ai[2] += 0.1f;
					}
					if (npc.life <= (npc.lifeMax * 0.5f) || CalamityWorld.bossRushActive)
					{
						npc.ai[2] += 0.15f;
					}
					if (npc.life <= (npc.lifeMax * 0.25f) || CalamityWorld.bossRushActive)
					{
						npc.ai[2] += 0.2f;
					}
					if (npc.life <= (npc.lifeMax * 0.1f) || CalamityWorld.bossRushActive)
					{
						npc.ai[2] += 0.5f;
					}
					if (npc.ai[2] >= 50f)
