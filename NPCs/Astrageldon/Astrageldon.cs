...
namespace CalamityMod.NPCs.Astrageldon
{
	...
	public class Astrageldon : ModNPC
	{
		...
		public override void AI()
		{
			...
			if (npc.ai[0] == 2f || npc.ai[0] >= 5f || (npc.ai[0] == 4f && npc.velocity.Y > 0f) || npc.GetGlobalNPC<CalamityGlobalNPC>(mod).enraged || (Config.BossRushXerocCurse && CalamityWorld.bossRushActive)) //fire fast, latent homing, projectiles while walking.  lasers in a circle pattern while teleporting
			{
				if (Main.netMode != 1)
				{
					...
					if (npc.localAI[0] >= 180f) //6 seconds 3 seconds 2 seconds
					{
						...
						if ((npc.ai[0] >= 5f && npc.ai[0] != 7) || npc.GetGlobalNPC<CalamityGlobalNPC>(mod).enraged || (Config.BossRushXerocCurse && CalamityWorld.bossRushActive)) //teleporting
						{
