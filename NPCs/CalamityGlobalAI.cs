    ...
    public class CalamityGlobalAI
    {
        ...
        public static bool BuffedKingSlimeAI(NPC npc, Mod mod)
        {
            ...
            if (npc.velocity.Y == 0f)
            {
                ...
                if (!flag8)
                {
                    ...
                    if ((double)npc.life < (double)npc.lifeMax * 0.2 || death)
                        npc.ai[0] += 3f;
        ...
        public static bool BuffedEyeofCthulhuAI(NPC npc, bool enraged, Mod mod)
        {
            ...
            else
            {
                ...
                if (npc.ai[1] == 0f)
                {
                    float num37 = ((enraged || configBossRushBoost) ? 8f : 5.5f) + 3.5f * (0.85f - lifeRatio);
                    float num38 = ((enraged || configBossRushBoost) ? 0.09f : 0.06f) + 0.025f * (0.85f - lifeRatio);
                    if (CalamityWorld.bossRushActive)
                    {
                        num37 *= 2f;
                        num38 *= 2f;
                ...
                else if (npc.ai[1] == 3f)
                {
                    ...
                    {
                        npc.TargetClosest(true);
						float speedBoost = death ? 2.45f : 3.5f * (0.7f - lifeRatio);
						float num48 = ((enraged || configBossRushBoost) ? 26f : 18f) + speedBoost;
                        if (CalamityWorld.bossRushActive)
                            num48 *= 1.5f;
                ...
                {
                    float num62 = 600f;
					float speedBoost = death ? 2f : 4f * (0.5f - lifeRatio);
					float speedBoost2 = death ? 0.05f : 0.1f * (0.5f - lifeRatio);
					float num63 = ((enraged || configBossRushBoost) ? 12f : 8f) + speedBoost;
                    float num64 = ((enraged || configBossRushBoost) ? 0.4f : 0.25f) + speedBoost2;
                    if (CalamityWorld.bossRushActive)
                    {
                        num63 *= 2f;
                        num64 *= 2f;
        ...
        public static bool BuffedEaterofWorldsAI(NPC npc, Mod mod)
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient && !phase5)
            {
                ...
                else if (npc.type == NPCID.EaterofWorldsHead)
                {
                    calamityGlobalNPC.newAI[0] += 1f;
                    float timer = 90f;
					float shootBoost = death ? 18f : lifeRatio * 180f;
                    timer += shootBoost;
			...
			float velocityBoost = death ? 2.4f : 2.4f * (1f - lifeRatio);
			float accelerationBoost = death ? 0.03f : 0.03f * (1f - lifeRatio);
        ...
        public static bool BuffedBrainofCthulhuAI(NPC npc, bool enraged, Mod mod)
        {
            ...
            if (npc.ai[0] < 0f)
            {
                ...
                bool phase4 = lifeRatio < 0.5f || death;
        ...
        public static bool BuffedQueenBeeAI(NPC npc, Mod mod)
        {
            ...
            else if (npc.ai[0] == 1f)
            {
                ...
                if (Collision.CanHit(vector76, 1, 1, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height) && spawnBee)
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        if (npc.life < (double)npc.lifeMax * 0.66 || death)
                            spawnType = 210;
            ...
            else if (npc.ai[0] == 3f)
            {
                ...
                npc.ai[1] += 1f;
                bool shoot = false;
                if ((double)npc.life < (double)npc.lifeMax * 0.33 || death || enrage)
                {
                    if (npc.ai[1] % 15f == 14f)
                        shoot = true;
                }
                ...
                if (shoot && npc.position.Y + (float)npc.height < Main.player[npc.target].position.Y && Collision.CanHit(vector78, 1, 1, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        if ((double)npc.life < (double)npc.lifeMax * 0.33 || enrage || death)
                            num624 += 3f;
        ...
        public static bool BuffedSkeletronAI(NPC npc, bool enraged, Mod mod)
        {
            ...
            if (handsDead || phase2)
            {
                ...
                npc.ai[3] += 1f + ((phase2 && handsDead) ? 0.5f : 0f) - (handsDead ? 0f : 0.5f);
            ...
            else if (npc.ai[1] == 1f)
            {
				...
				float phaseChangeRateBoost = death ? 0.5f : 0.5f * (1f - lifeRatio);
				npc.ai[2] += 1f + phaseChangeRateBoost;
            ...
            if (npc.ai[2] == 0f || npc.ai[2] == 3f)
            {
                ...
                else
                {
                    ...
                    if (npc.position.X + (float)(npc.width / 2) > Main.npc[(int)npc.ai[1]].position.X + (float)(Main.npc[(int)npc.ai[1]].width / 2) - 200f * npc.ai[0])
                    {
                        ...
                        npc.velocity.X -= CalamityWorld.bossRushActive ? 0.22f : 0.14f;
                        if (npc.velocity.X > maxX)
                            npc.velocity.X = maxX;
                    }
        ...
        public static bool BuffedWallofFleshAI(NPC npc, bool enraged, Mod mod)
        {
            ...
            {
				// Range of 1 to 11
				float spawnBoost = death ? 1f : lifeRatio * 10f;
        ...
        public static bool BuffedRetinazerAI(NPC npc, bool enraged, Mod mod)
        {
            ...
            else if (npc.ai[0] == 0f)
            {
                ...
                if ((double)npc.life < (double)npc.lifeMax * 0.7 || targetFloatingUp || death)
                {
                    npc.ai[0] = 1f;
                    npc.ai[1] = 0f;
                    npc.ai[2] = 0f;
        ...
        public static bool BuffedSkeletronPrimeAI(NPC npc, bool enraged, Mod mod)
        {
            ...
            // Phases
            bool phase2 = (double)npc.life < (double)npc.lifeMax * 0.66 || (death && allArmsDead);
            bool phase3 = (double)npc.life < (double)npc.lifeMax * 0.33 || (death && allArmsDead);
        ...
        public static bool BuffedPlanterasTentacleAI(NPC npc, Mod mod)
        {
            ...
            float num781 = death ? 1f : 1f - (float)npc.life / (float)npc.lifeMax;
        ...
        public static bool BuffedGolemAI(NPC npc, bool enraged, Mod mod)
        {
            ...
            bool phase4 = lifeRatio < 0.25f || death;
            ...
            if (npc.ai[0] == 0f)
            {
                ...
                if (npc.velocity.Y == 0f)
                {
                    ...
                    else if (npc.ai[1] == -1f)
                    {
						...
						float velocityBoost = death ? 8f : 8f * (1f - lifeRatio);
        ...
        public static bool BuffedGolemHeadAI(NPC npc, bool enraged, Mod mod)
        {
            ...
            else if (npc.ai[0] == 1f)
            {
				...
				// Lasers
				float shootBoost2 = death ? 3f : 3f * (1f - lifeRatio);
        ...
        public static bool BuffedDukeFishronAI(NPC npc, bool enraged, Mod mod)
        {
			...
			bool death = CalamityWorld.death || CalamityWorld.bossRushActive;
			bool phase2 = (double)npc.life <= (double)npc.lifeMax * 0.66 || death;
            bool phase3 = (double)npc.life <= (double)npc.lifeMax * (death ? 0.33 : 0.2);
        ...
        public static bool BuffedCultistAI(NPC npc, bool enraged, Mod mod)
        {
            ...
            bool phase3 = lifeRatio < 0.6f || death;
            bool phase4 = lifeRatio < 0.4f || (death && lifeRatio < 0.7f);
            bool phase5 = lifeRatio < 0.2f || (death && lifeRatio < 0.4f);
            ...
            float iceMistSpeed = death ? 6.5f : 6f;

            int fireballFireRate = 12;

            float fireballSpeed = (death ? 7f : 6.5f) - (isCultist ? 0f : 3f);
        ...
        public static bool BuffedAncientDoomAI(NPC npc, Mod mod)
        {
            ...
            {
                if (Main.npc[(int)npc.ai[0]].life < Main.npc[(int)npc.ai[0]].lifeMax / 2 || death)
                {
                    num1498 = 2;
                }
                if (Main.npc[(int)npc.ai[0]].life < Main.npc[(int)npc.ai[0]].lifeMax / 4 || death)
                {
                    num1498 = 3;
                }
            }
                ...
                if (npc.ai[0] == 0f)
                {
                    ...
                    if (value4.Length() > 20f)
                    {
                        ...
                        if (Main.npc[(int)npc.localAI[2]].ai[0] == 1f)
                            velocity = 8f;
						if (Main.player[npc.target].mount.Active && Main.player[npc.target].mount.Type == 10)
							velocity *= 1.5f;
		...
		public static bool BuffedSlimeAI(NPC npc, Mod mod)
		{
			...
			if (npc.type == 184)
			{
				...
				if (!npc.wet && !Main.player[npc.target].npcTypeNoAggro[npc.type])
				{
					...
					if (num10 < 120f && Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height) && npc.velocity.Y == 0f)
					{
						npc.ai[0] = -40f;
						if (npc.velocity.Y == 0f)
						{
			...
			if (npc.type == 204)
			{
				...
				if (!npc.wet && !Main.player[npc.target].npcTypeNoAggro[npc.type])
				{
					...
					if (num16 < 200f && Collision.CanHit(new Vector2(npc.position.X, npc.position.Y - 20f), npc.width, npc.height + 20, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height) && npc.velocity.Y == 0f)
					{
						npc.ai[0] = -40f;
						if (npc.velocity.Y == 0f)
						{
		...
		public static bool BuffedFighterAI(NPC npc, Mod mod)
        {
			...
			{
				int num7 = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, NPCID.ForceBubble, npc.whoAmI, 0f, 0f, 0f, 0f, 255);
			...
			if (npc.type == NPCID.MartianOfficer)
			{
				...
				if (num8 != -1 && Main.npc[num8].active && Main.npc[num8].type == NPCID.ForceBubble)
				{
					npc.dontTakeDamage = true;
				}
				else
			...
			if (npc.type == NPCID.GoblinSummoner)
			{
				...
				{
					npc.knockBackResist = 0f;
					npc.defense = (int)((double)npc.defDefense * 1.3);
					npc.noGravity = true;
					npc.noTileCollide = true;
					if (npc.velocity.X < 0f)
					{
					...
					if (Main.netMode != 1)
					{
						...
						if (npc.localAI[3] > (float)Main.rand.Next(20, 120))
						{
							...
							NPC.NewNPC((int)vector7.X, (int)vector7.Y, NPCID.ChaosBall, 0, 0f, 0f, 0f, 0f, 255);
						}
					}
				}
				else
					...
					else
					{
						if (npc.ai[3] == 1f)
						{
							...
							if (npc.ai[2] == 30f)
							{
								if (Main.netMode != 1)
								{
									NPC.NewNPC((int)center3.X, (int)center3.Y + 18, NPCID.ShadowFlameApparition, 0, 0f, 0f, 0f, 0f, 255);
								}
							}
						...
						npc.ai[2] += 1f;
						int num27 = 10;
						if (npc.velocity.Y == 0f && NPC.CountNPCS(NPCID.ShadowFlameApparition) < num27)
						{
							if (npc.ai[2] >= 180f)
							{
								npc.ai[2] = 0f;
						...
						{
							if (NPC.CountNPCS(NPCID.ShadowFlameApparition) >= num27)
							{
								npc.ai[2] += 1f;
							}
							if (npc.ai[2] >= 360f)
			...
			if (npc.type == NPCID.SolarDrakomire)
			{
				...
				else if (npc.ai[2] < 0f && npc.ai[2] > (float)(-(float)num30))
				{
					...
					if (npc.ai[2] == (float)(-(float)num31) || npc.ai[2] == (float)(-(float)num31 - 8) || npc.ai[2] == (float)(-(float)num31 - 16))
					{
						...
						if (Main.netMode != 1)
						{
							NPC.NewNPC((int)npc.Center.X + npc.spriteDirection * 45, (int)npc.Center.Y + 8, NPCID.SolarFlare, 0, 0f, 0f, 0f, 0f, npc.target);
						}
					}
					return false;
				}
			...
			if (npc.type == NPCID.VortexHornet)
			{
				...
				if (npc.localAI[0] >= 600f && Main.netMode != 1)
				{
			...
			if (Main.netMode != 1 && npc.type == NPCID.Lihzahrd && (double)npc.life <= (double)npc.lifeMax * 0.9)
			{
				npc.Transform(NPCID.LihzahrdCrawler);
			}
			if (Main.netMode != 1 && npc.type == NPCID.Nutcracker && (double)npc.life <= (double)npc.lifeMax * 0.9)
			{
			...
			bool flag4 = false;
			bool flag5 = true;
			if (npc.type == 343 || npc.type == 47 || npc.type == 67 || npc.type == 109 || npc.type == 110 || npc.type == 111 || npc.type == 120
				|| npc.type == 163 || npc.type == 164 || npc.type == 239 || npc.type == 168 || npc.type == 199 || npc.type == 206 || npc.type == 214
				|| npc.type == 215 || npc.type == 216 || npc.type == 217 || npc.type == 218 || npc.type == 219 || npc.type == 220 || npc.type == 226
				|| npc.type == 243 || npc.type == 251 || npc.type == 257 || npc.type == 258 || npc.type == 290 || npc.type == 291 || npc.type == 292
				|| npc.type == 293 || npc.type == 305 || npc.type == 306 || npc.type == 307 || npc.type == 308 || npc.type == 309 || npc.type == 348
				|| npc.type == 349 || npc.type == 350 || npc.type == 351 || npc.type == 379 || (npc.type >= 430 && npc.type <= 436)
				|| (npc.type == 380 || npc.type == 381 || npc.type == 382 || npc.type == 383 || npc.type == 386 || npc.type == 391
				|| (npc.type >= 449 && npc.type <= 452)) || (npc.type == 466 || npc.type == 464 || npc.type == 166 || npc.type == 469 || npc.type == 468
				|| npc.type == 471 || npc.type == 470 || npc.type == 480 || npc.type == 481 || npc.type == 482 || npc.type == 411 || npc.type == 424
				|| npc.type == 409 || (npc.type >= 494 && npc.type <= 506)) || (npc.type == 425 || npc.type == 427 || npc.type == 426 || npc.type == 428
				|| npc.type == 508 || npc.type == 415 || npc.type == 419 || npc.type == 520 || (npc.type >= 524 && npc.type <= 527)) || npc.type == 528
				|| npc.type == 529 || npc.type == 530 || npc.type == 532 || npc.type == ModContent.NPCType<StormlionCharger>())
			{
				flag5 = false;
			}
			bool flag6 = false;
		...
		IL_2CAD:
			...
			else if (npc.type == NPCID.VortexRifleman)
			{
				...
				if (npc.velocity.Y != 0f && npc.ai[2] == 1f)
				{
					...
					{
						float num81 = Main.player[npc.target].Center.X - (float)(npc.direction * 600) - npc.Center.X;
						...
						if (num81 < 0f && npc.velocity.X > -7f)
						{
							npc.velocity.X = npc.velocity.X - 0.12f;
						}
						else if (num81 > 0f && npc.velocity.X < 7f)
						{
							npc.velocity.X = npc.velocity.X + 0.12f;
						}
						if (npc.velocity.X > 8f)
						...
						if (num82 < -20f && npc.velocity.Y > -7f)
						{
							npc.velocity.Y = npc.velocity.Y - 0.35f;
						}
						else if (num82 > 20f && npc.velocity.Y < 7f)
						{
							npc.velocity.Y = npc.velocity.Y + 0.35f;
						}
					}
				...
				if (Main.netMode != 1)
				{
					...
					if (npc.localAI[2] >= (float)(240 + Main.rand.Next(240)) && npc.Distance(Main.player[npc.target].Center) < 600f && Math.Abs(npc.DirectionTo(Main.player[npc.target].Center).Y) < 0.5f && Collision.CanHitLine(npc.Center, 0, 0, Main.player[npc.target].Center, 0, 0))
					{
						...
						Vector2 vector16 = npc.DirectionTo(Main.player[npc.target].Center) * 9f;
						if (vector16.HasNaNs())
						{
							vector16 = new Vector2((float)(npc.direction * 10), 0f);
						}
						int num84 = 50;
			...
			else if (npc.type == NPCID.VortexHornet)
			{
				...
				if (npc.velocity.Y != 0f && npc.ai[2] == 1f)
				{
					...
					if (Collision.CanHit(npc.Center, 0, 0, Main.player[npc.target].Center, 0, 0))
					{
						...
						if (num86 < -20f && npc.velocity.X > -8f)
						{
							npc.velocity.X = npc.velocity.X - 0.025f;
						}
						else if (num86 > 20f && npc.velocity.X < 8f)
						{
							npc.velocity.X = npc.velocity.X + 0.025f;
						}
						if (npc.velocity.X > 8f)
						...
						if (num87 < -20f && npc.velocity.Y > -8f)
						{
							npc.velocity.Y = npc.velocity.Y - 0.25f;
						}
						else if (num87 > 20f && npc.velocity.Y < 8f)
						{
							npc.velocity.Y = npc.velocity.Y + 0.25f;
						}
					}
			...
			else if (npc.type == NPCID.VortexHornetQueen)
			{
				...
				if (npc.velocity.Y != 0f)
				{
					...
					if (Collision.CanHit(npc.Center, 0, 0, Main.player[npc.target].Center, 0, 0))
					{
						float num89 = Main.player[npc.target].Center.X - (float)(npc.direction * 450) - npc.Center.X;
						...
						if (num89 < 40f && npc.velocity.X > -7f)
						{
							npc.velocity.X = npc.velocity.X - 0.25f;
						}
						else if (num89 > 40f && npc.velocity.X < 7f)
						{
							npc.velocity.X = npc.velocity.X + 0.25f;
						}
						if (npc.velocity.X > 8f)
			...
			if (npc.type == NPCID.WallCreeper && Main.netMode != 1 && npc.velocity.Y == 0f)
			{
				...
				if (flag9)
				{
					npc.Transform(NPCID.WallCreeperWall);
				}
			}
			if (npc.type == NPCID.BloodCrawler && Main.netMode != 1 && npc.velocity.Y == 0f)
			{
			...
			if (npc.type == NPCID.DesertScorpionWalk && Main.netMode != 1 && npc.velocity.Y == 0f)
			{
			...
			if (npc.type == NPCID.BlackRecluse && Main.netMode != 1 && npc.velocity.Y == 0f)
			{
			...
			if (npc.type == NPCID.JungleCreeper && Main.netMode != 1 && npc.velocity.Y == 0f)
			{
			...
			if (npc.type == NPCID.IceGolem)
			{
				...
				npc.ai[2] += 1f;
				if (Main.netMode != 1 && npc.ai[2] >= 90f && npc.velocity.Y == 0f && !Main.player[npc.target].dead && !Main.player[npc.target].frozen && ((npc.direction > 0 && npc.Center.X < Main.player[npc.target].Center.X) || (npc.direction < 0 && npc.Center.X > Main.player[npc.target].Center.X)) && Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
				{
						...
						while (!flag14 && num130 < 100)
						{
							...
							for (int num134 = Main.rand.Next(num126 - num129, num126 + num129); num134 < num126 + num129; num134++)
							{
								if ((num134 < num126 - num131 || num134 > num126 + num131 || num133 < num125 - num131 || num133 > num125 + num131) && (num134 < num128 - num132 || num134 > num128 + num132 || num133 < num127 - num132 || num133 > num127 + num132) && Main.tile[num133, num134].nactive())
								{
									...
									{
										int num135 = NPC.NewNPC(num133 * 16 - npc.width / 2, num134 * 16, NPCID.MartianTurret, 0, 0f, 0f, 0f, 0f, 255);
			...
			if (npc.type == NPCID.GigaZapper)
			{
				...
				else
				{
					...
					if (npc.ai[2] == 20f && Main.netMode != 1)
					{
						...
						Projectile.NewProjectile(npc.Center.X, npc.Center.Y + 8f, (float)(npc.direction * 8), 0f, 437, 25, 1f, Main.myPlayer, 0f, 0f);
					}
				}
			}
			if (npc.type == 110 || npc.type == 111 || npc.type == 206 || npc.type == 214 || npc.type == 215 || npc.type == 216 || npc.type == 290 || npc.type == 291 || npc.type == 292 || npc.type == 293 || npc.type == 350 || npc.type == 379 || npc.type == 380 || npc.type == 381 || npc.type == 382 || (npc.type >= 449 && npc.type <= 452) || (npc.type == 468 || npc.type == 481 || npc.type == 411 || npc.type == 409 || (npc.type >= 498 && npc.type <= 506)) || npc.type == 424 || npc.type == 426 || npc.type == 520)
			...
			if (flag22)
			{
				...
				else
				{
					...
					{
						npc.velocity.X = npc.velocity.X * 1.25f;
						npc.velocity.Y = npc.velocity.Y * 1.15f;
					}
					if (npc.type == 460 && npc.velocity.Y < 0f)
					{
						npc.velocity.X = npc.velocity.X * 1.35f;
						npc.velocity.Y = npc.velocity.Y * 1.15f;
					}
				}
			}
			...
			if (Main.netMode != 1 && npc.type == NPCID.ChaosElemental && npc.ai[3] >= (float)num35)
			{
		...
		public static bool BuffedFlyingAI(NPC npc, Mod mod)
		{
			...
			if (npc.type == 6 || npc.type == 23 || npc.type == 94 || npc.type == 139 || npc.type == 173 || npc.type == 205)
			{
				...
				if (npc.collideY)
				{
					npc.netUpdate = true;
					npc.velocity.Y = npc.oldVelocity.Y * -num11;
					if (npc.velocity.Y > 0f && (double)npc.velocity.Y < 2.5)
					{
						npc.velocity.Y = 3f;
					}
					if (npc.velocity.Y < 0f && (double)npc.velocity.Y > -2.5)
		...
		public static bool BuffedFlyingWeaponAI(NPC npc, Mod mod)
		{
			...
			if (npc.ai[0] == 0f)
			{
				...
				num321 *= num323;
				num322 *= num323;
				npc.velocity.X = num321;
				npc.velocity.Y = num322;
		...
		public static bool BuffedAngryNimbusAI(NPC npc, Mod mod)
		{
			...
			float num723 = Main.player[npc.target].Center.X - vector90.X;
			float num724 = Main.player[npc.target].Center.Y - vector90.Y - 300f;
			float num725 = (float)Math.Sqrt((double)(num723 * num723 + num724 * num724));
			if (num725 < 20f)
			{
				num723 = npc.velocity.X;
				num724 = npc.velocity.Y;
			}
			else
			{
			...
			if (npc.velocity.X < num723)
			{
				npc.velocity.X = npc.velocity.X + num722;
				if (npc.velocity.X < 0f && num723 > 0f)
				{
					npc.velocity.X = npc.velocity.X + num722 * 2f;
				}
			}
			else if (npc.velocity.X > num723)
			{
				npc.velocity.X = npc.velocity.X - num722;
				if (npc.velocity.X > 0f && num723 < 0f)
				{
			...
			if (npc.velocity.Y < num724)
			{
				npc.velocity.Y = npc.velocity.Y + num722;
				if (npc.velocity.Y < 0f && num724 > 0f)
				{
					npc.velocity.Y = npc.velocity.Y + num722 * 2f;
				}
			}
			else if (npc.velocity.Y > num724)
			{
				npc.velocity.Y = npc.velocity.Y - num722;
				if (npc.velocity.Y > 0f && num724 < 0f)
				{
			...
			if (npc.position.X + (float)npc.width > Main.player[npc.target].position.X - 150f && npc.position.X < Main.player[npc.target].position.X + (float)Main.player[npc.target].width + 150f && npc.position.Y + (float)npc.height < Main.player[npc.target].position.Y && Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height) && Main.netMode != 1)
			{
				npc.ai[0] += 1f;
				if (npc.ai[0] % 8f == 0f)
				{
					int num726 = (int)(npc.position.X + 10f + (float)Main.rand.Next(npc.width - 20));
					int num727 = (int)(npc.position.Y + (float)npc.height + 4f);
					float velocityX = (float)Main.rand.Next(-3, 4) * (Main.rand.NextFloat() - 0.5f);
					Projectile.NewProjectile((float)num726, (float)num727, 0f, 5f, ProjectileID.RainNimbus, 20, 0f, Main.myPlayer, 0f, 0f);
					if (npc.ai[0] % 16f == 0f)
						Projectile.NewProjectile((float)num726, (float)num727, velocityX, 5f, ProjectileID.FrostShard, 20, 0f, Main.myPlayer, 0f, 0f);
				}
				if (npc.ai[0] >= 607f)
					npc.ai[0] = 0f;
			}
		...
		#region Spore AI
		public static bool BuffedSporeAI(NPC npc, Mod mod, bool death)
		{
			if (npc.timeLeft > 5)
			{
				npc.timeLeft = 5;
			...
			npc.TargetClosest(true);
			float acceleration = death ? 0.25f : 0.2f;
			float velocity = death ? 7f : 5f;
		...
		public static bool BuffedGraniteElementalAI(NPC npc, Mod mod)
		{
			...
			else if (npc.ai[0] == 3f)
			{
				...
				if (num1387 < num1388 || num1387 > 800f || Collision.CanHit(npc.Center, 1, 1, Main.player[npc.target].Center, 1, 1))
				{
					npc.ai[0] = 0f;
				}
			}
