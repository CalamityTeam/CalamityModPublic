    ...
    public class Providence : ModNPC
    {
                ...
                case (int)Phase.Crystal:
                    ...
                    if (NPC.ai[1] >= crystalPhaseTime)
                    {
                        if (NPC.ai[1] == crystalPhaseTime)
                        {
                            ...
                            Vector2 dustLineStart = new Vector2(NPC.Center.X, NPC.Center.Y + 48f * NPC.scale);
                    ...
                    if (NPC.ai[2] < 120f)
                    {
                        if (NPC.ai[2] >= 40f)
                        {
                            ...
                            for (int d = 0; d < 1 + extraDustAmt; d++)
                            {
                                ...
                                Vector2 dustPos = new Vector2(NPC.Center.X, NPC.Center.Y + 48f * NPC.scale) + ((float)Main.rand.NextDouble() * MathHelper.TwoPi).ToRotationVector2() * dustPosOffset / 2f;
                    ...
                    else if (NPC.ai[2] < (revenge ? 220f : 300f))
                    {
                        if (NPC.ai[2] == 120f)
                        {
                            ...
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                ...
                                Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center.X, NPC.Center.Y + 48f * NPC.scale, velocity.X, velocity.Y, ModContent.ProjectileType<ProvidenceHolyRay>(), holyLaserDamage, 0f, Main.myPlayer, beamDirection * MathHelper.TwoPi / rotation, NPC.whoAmI);
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            if (attackerAlive)
            {
                ...
                float drawOffsetAmt = (AIState == (int)Phase.FlameCocoon || AIState == (int)Phase.SpearCocoon) ? 20f : 48f;
