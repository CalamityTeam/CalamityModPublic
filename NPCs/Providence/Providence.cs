	...
	public class Providence : ModNPC
	{
				...
				if (Main.netMode != 1)
				{
					...
					if (npc.ai[3] >= (float)num864)
					{
						...
						if (!WorldGen.SolidTile(i2, j2))
						{
							...
							Projectile.NewProjectile(vector113.X, vector113.Y, speedX2, num865, mod.ProjectileType("HolyBomb"), fireDamage, 0f, Main.myPlayer, 0f, 0f);
						}
					}
				}
				if (Main.netMode != 1)
		...
		public override void NPCLoot()
		{
			...
			if (Main.expertMode)
			{
				...
				else if (Main.player[npc.target].ZoneHoly)
				{
