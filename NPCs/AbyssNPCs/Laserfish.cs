    ...
    public class Laserfish : ModNPC
	{
		...
		public override void AI()
		{
			...
			if (npc.wet)
			{
				...
				if (Main.player[npc.target].wet && !Main.player[npc.target].dead &&
                    ...
                    (Main.player[npc.target].Center - npc.Center).Length() < Main.player[npc.target].GetCalamityPlayer().GetAbyssAggro(400f, 200f))
                {
					flag14 = true;
				}
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			...
			if (npc.life <= 0)
			{
				...
				Gore.NewGore(npc.Center, npc.velocity, mod.GetGoreSlot("Gores/Laserfish"), 1f);
				Gore.NewGore(npc.Center, npc.velocity, mod.GetGoreSlot("Gores/Laserfish2"), 1f);
				Gore.NewGore(npc.Center, npc.velocity, mod.GetGoreSlot("Gores/Laserfish3"), 1f);
			}
		}
	}
}
