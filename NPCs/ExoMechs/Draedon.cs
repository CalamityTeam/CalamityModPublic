        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            ...
            if (Main.netMode != NetmodeID.Server)
            {
                for (int i = 1; i <= 4; i++)
                {
                    ...
                    Gore.NewGoreDirect(NPC.Center + goreSpawnOffset, draedonPieceVelocity, Mod.Find<ModGore>($"Draedon{i}").Type);

                    goreSpawnOffset = Main.rand.NextVector2Circular(18f, 18f);
                    Gore.NewGoreDirect(NPC.Center + goreSpawnOffset, chairPieceVelocity, Mod.Find<ModGore>($"Chair{i}").Type);
                }
            }
        }
