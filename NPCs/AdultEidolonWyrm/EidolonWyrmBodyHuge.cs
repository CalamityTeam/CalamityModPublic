...
namespace CalamityMod.NPCs.AdultEidolonWyrm
{
    public class EidolonWyrmBodyHuge : ModNPC
    {
			...
			if (spawnAncientLights && Main.netMode != NetmodeID.MultiplayerClient)
			{
				if (Vector2.Distance(npc.Center, Main.player[Main.npc[(int)npc.ai[2]].target].Center) > 160f)
				{
					...
					if (npc.ai[3] % divisor == 0f && npc.localAI[0] >= spawnAncientLightGateValue)
					{
						...
						Vector2 velocity = destination.SafeNormalize(Vector2.UnitX * npc.spriteDirection) * lightVelocity;
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            ...
            vector -= new Vector2(ModContent.GetTexture("CalamityMod/NPCs/AdultEidolonWyrm/EidolonWyrmBodyGlowHuge").Width, ModContent.GetTexture("CalamityMod/NPCs/AdultEidolonWyrm/EidolonWyrmBodyGlowHuge").Height) * 0.5f;
            vector += vector11 * 1f + new Vector2(0f, 4f + npc.gfxOffY);
            Color color = new Color(127, 127, 127, 0).MultiplyRGBA(Color.LightYellow) * npc.Opacity;
            Main.spriteBatch.Draw(ModContent.GetTexture("CalamityMod/NPCs/AdultEidolonWyrm/EidolonWyrmBodyGlowHuge"), vector,
