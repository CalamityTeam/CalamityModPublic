        ...
        public override void PostDraw(NPC npc, SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            else if (CalamityLists.DestroyerIDs.Contains(npc.type))
            {
                ...
                if (npc.ai[2] == 0f && segmentDrawColor != Color.Black)
                {
                    ...
                    Texture2D glowTexture = CalamityConfig.Instance.NewVanillaTextures ? TextureLoadingSystem.DestroyerGlowmasks[0].Value : TextureAssets.Dest[0].Value;
                    switch (npc.type)
                    {
                        default:
                        case NPCID.TheDestroyer:
                        ...
                        case NPCID.TheDestroyerBody:
                            glowTexture = CalamityConfig.Instance.NewVanillaTextures ? TextureLoadingSystem.DestroyerGlowmasks[1].Value : TextureAssets.Dest[1].Value;
                            break;

                        case NPCID.TheDestroyerTail:
                            glowTexture = CalamityConfig.Instance.NewVanillaTextures ? TextureLoadingSystem.DestroyerGlowmasks[2].Value : TextureAssets.Dest[2].Value;
                            break;
                    }
            ...
            if (revenge)
            {
                ...
                else if (npc.type == NPCID.WallofFleshEye && Main.wofNPCIndex >= 0)
                {
                    ...
                    {
                        Texture2D glowTexture = CalamityConfig.Instance.NewVanillaTextures ? TextureLoadingSystem.WallOfFleshEyeGlowmask.Value : TextureAssets.Npc[npc.type].Value;
                        Vector2 halfSize = npc.frame.Size() / 2;
                        SpriteEffects spriteEffects = SpriteEffects.None;
                        if (npc.spriteDirection == 1)
                            spriteEffects = SpriteEffects.FlipHorizontally;
                ...
                {
                    Texture2D npcTexture = (masterMode && revenge && npc.type == NPCID.SkeletronPrime) ? TextureLoadingSystem.ChadPrime.Value : TextureAssets.Npc[npc.type].Value;
                    int frameHeight = npcTexture.Height / Main.npcFrameCount[npc.type];
                    ...
                    if (masterMode && revenge)
                    {
                        ...
                        Texture2D glowTexture = npc.type == NPCID.SkeletronPrime ? TextureLoadingSystem.ChadPrimeEyeGlowmask.Value : SkeletronPrime2.EyeTexture.Value;
