    ...
    public class CrimulanSlimeGod : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (lifeRatio <= 0.5f && Main.netMode != NetmodeID.MultiplayerClient && expertMode)
            {
                ...
                NPC.NewNPC(NPC.GetSource_FromAI(), (int)spawnAt.X + 30, (int)spawnAt.Y, ModContent.NPCType<SplitCrimulanSlimeGod>());
                if (Main.getGoodWorld && NPC.CountNPCS(ModContent.NPCType<SplitEbonianSlimeGod>()) < 3) // move to zenith seed later. split into 3 slimes if the other large slime hasn't split yet
                {
