...
namespace CalamityMod.NPCs.SlimeGod
{
    ...
    public class SlimeGodSplit : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            npc.noTileCollide = false;
            npc.noGravity = false;
            if (npc.ai[0] != 6f && Main.player[npc.target].dead)
            {
                npc.TargetClosest(true);
            ...
            else if (npc.ai[0] == 1f)
            {
                ...
                if (npc.velocity.Y == 0f)
                {
                    ...
                    float num1879 = 15f;
                    float num1880 = CalamityWorld.bossRushActive ? 16f : 7f;
					if (revenge)
					{
						float moveBoost = death ? 12f : 12f * (1f - (float)npc.life / (float)npc.lifeMax);
				...
				if (revenge)
				{
					npc.ai[2] += (death ? 1f : 1f * (1f - (float)npc.life / (float)npc.lifeMax));
				}
				if (npc.ai[2] >= 120f && npc.velocity.Y == 0f && Main.netMode != NetmodeID.MultiplayerClient)
                {
            ...
            else if (npc.ai[0] == 3f)
            {
                ...
                else
                {
                    ...
                    vector272.Normalize();
                    vector272 *= CalamityWorld.bossRushActive ? 24f : 16f;
