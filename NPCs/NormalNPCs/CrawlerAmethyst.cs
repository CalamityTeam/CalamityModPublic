...
namespace CalamityMod.NPCs.NormalNPCs
{
    public class CrawlerAmethyst : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			banner = npc.type;
			bannerItem = mod.ItemType("AmethystCrawlerBanner");
		}
		...
		{
			if (spawnInfo.playerSafe || spawnInfo.player.GetCalamityPlayer().ZoneAbyss || spawnInfo.player.GetCalamityPlayer().ZoneSunkenSea)
			{
				return 0f;
			}
		...
		{
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.Amethyst, Main.rand.Next(2, 5));
		}
	}
}
