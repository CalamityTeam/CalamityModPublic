...
{
	public class DetonatingFlare3 : ModNPC
	{
        float num = 10f;
        float num2 = 0.25f;
		...
		{
			DisplayName.SetDefault("Angery Flame");
		...
		public override void AI()
		{
            ...
            {
                npc.localAI[0] = 0f;
                int damage = Main.expertMode ? 106 : 125; //600
                Vector2 vector173 = Vector2.Normalize(Main.player[npc.target].Center - npc.Center) * (float)(npc.width + 20) / 2f + npc.Center;
                float speed = 0.01f;
                Vector2 vectorShoot = new Vector2(npc.position.X + (float)npc.width * 0.5f, npc.position.Y + (float)npc.height * 0.5f + 30f);
                float playerX = Main.player[npc.target].position.X + (float)Main.player[npc.target].width * 0.5f - vectorShoot.X;
                float playerY = Main.player[npc.target].position.Y - vectorShoot.Y;
                float playerXY = (float)Math.Sqrt((double)(playerX * playerX + playerY * playerY));
                playerXY = speed / playerXY;
                playerX *= playerXY;
                playerY *= playerXY;
                Projectile.NewProjectile((int)vector173.X, (int)vector173.Y, playerX, playerY, mod.ProjectileType("FlareDust2"), damage, 0f, Main.myPlayer, 0f, 0f);
                Projectile.NewProjectile(vector.X, vector.Y, num4, num5, 467, damage, 0f, Main.myPlayer, 0f, 0f);
            }
