...
namespace CalamityMod.NPCs.SupremeCalamitas
{
    public class SupremeCalamitas : ModNPC
    {
        ...
        public const int PermafrostPhotonRipperDamage = 3725;
        ...
        internal static void LoadHeadIcons()
        {
            ...
            string permafrostIconPath = "CalamityMod/NPCs/TownNPCs/DILF_Head";
            ...
            CalamityMod.Instance.AddBossHeadTexture(permafrostIconPath, -1);
            permafrostHeadIconIndex = ModContent.GetModBossHeadSlot(permafrostIconPath);
        }
        ...
        public override void AI()
        {
            ...
            bool zenithAI = Main.zenithWorld && (!permafrost || (CalamityWorld.LegendaryMode && revenge && permafrost));
                ...
                if (!player.active || player.dead)
                {
                    ...
                    if (NPC.alpha >= 230)
                    {
                        if (DownedBossSystem.downedCalamitas && !bossRush)
                        {
                            ...
                            Dust.QuickDustLine(NPC.Center, initialRitualPosition, 500f, permafrost ? Color.Pink : Color.Red);
                            NPC.Center = initialRitualPosition;

                            // Make the town NPC spawn.
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                                NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X, (int)NPC.Center.Y + 12, permafrost ? ModContent.NPCType<DILF>() : ModContent.NPCType<WITCH>());
                        }
                    ...
                    for (int i = 0; i < MathHelper.Lerp(2f, 6f, 1f - NPC.Opacity); i++)
                    {
                        ...
                        brimstoneFire.color = permafrost ? Color.Pink : Color.Red;
            ...
            else if (!startBattle)
            {
                ...
                Particle pulse = new DirectionalPulseRing(NPC.Center, Vector2.Zero, permafrost ? Color.Pink : Color.Red, new Vector2(1f, 1f), 0, 0.1f, 5f, 15);
                GeneralParticleHandler.SpawnParticle(pulse);
                Particle pulse2 = new DirectionalPulseRing(NPC.Center, Vector2.Zero, permafrost ? Color.Pink : Color.Lerp(Color.Red, Color.Magenta, 0.3f), new Vector2(1f, 1f), 0, 0.05f, 4f, 18);
            ...
            if (bulletHellCounter2 < ThirdBulletHellEndValue && startThirdAttack)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    // Permafrost uses Photon Ripper
                    if (bulletHellCounter2 == SecondBulletHellEndValue + 1 && permafrost)
                        Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, Vector2.One, ModContent.ProjectileType<PermafrostAbsoluteZeroProjectile>(), PermafrostPhotonRipperDamage, 0f, Main.myPlayer, 0f, 0f, NPC.whoAmI);
            ...
            {
                Particle pulse = new DirectionalPulseRing(NPC.Center, Vector2.Zero, permafrost ? Color.Pink : Color.Red, new Vector2(1f, 1f), 0, 0.1f, 4f, 17);
                GeneralParticleHandler.SpawnParticle(pulse);
                Particle pulse2 = new DirectionalPulseRing(NPC.Center, Vector2.Zero, permafrost ? Color.Pink : Color.Lerp(Color.Red, Color.Magenta, 0.3f), new Vector2(1f, 1f), 0, 0.05f, 3f, 19);
                GeneralParticleHandler.SpawnParticle(pulse2);
                for (int i = 0; i < 30; i++)
                {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient) // More clustered attack
                {
                    ...
                    if (passedVar == 420) // Giant homing fireballs
                    {
                        ...
                        for (int i = 0; i < 40; i++)
                        {
                            ...
                            GlowOrbParticle orb = new GlowOrbParticle(safeBox.Center() + sparkVel * 2, sparkVel, false, 120, Main.rand.NextFloat(1.55f, 2.75f), permafrost ? Color.Magenta : Color.Lerp(Color.Red, Color.Magenta, 0.3f), true, true);
                            GeneralParticleHandler.SpawnParticle(orb);
                        }
            ...
            if (startFifthAttack)
            {
                if (gettingTired5)
                {
                    if (permafrost)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                int cryo = NPC.FindFirstNPC(ModContent.NPCType<Cryogen.Cryogen>());
                                if (cryo > -1)
                                {
                    ...
                    else // Scal gives up
                    {
                        ...
                        {
                            attackPause = 5;
                            Dust.QuickDustLine(NPC.Center, safeBox.Center() + new Vector2(0, -30), 500f, permafrost ? Color.Pink : Color.Red);
                            ...
                            {
                                Dust.QuickDustLine(NPC.Center, initialRitualPosition + new Vector2(0, -30), 500f, permafrost ? Color.Pink : Color.Red);
                ...
                else if (!gettingTired && lifeRatio <= 0.08f)
                {
                    ...
                    for (int i = 0; i < 40; i++)
                    {
                        Dust castFire = Dust.NewDustPerfect(NPC.Center + Main.rand.NextVector2Square(-70f, 70f), permafrost ? DustID.IceGolem : (int)CalamityDusts.Brimstone);
            ...
            if (NPC.life > 0)
            {
                ...
                if (lifeRatio < 0.45f && !enteredBrothersPhase)
                {
                    ...
                    if (preventionPause == 0)
                    {
                        ...
                        if (!teleport)
                        {
                            Dust.QuickDustLine(NPC.Center, player.Center + new Vector2(0, -155), 500f, permafrost ? Color.Pink : Color.Red);
                            NPC.velocity = Vector2.Zero;
                ...
                else
                {
                        ...
                        if (NPC.localAI[1] > 90f)
                        {
                            ...
                            if (randomShot == 0 && canFireSplitingFireball && !fireFireblastFirst)
                            {
                                ...
                                for (int i = 0; i < 9; i++) // fireblasts pre laugh
                                {
                                    ...
                                    GlowOrbParticle spark2 = new GlowOrbParticle(projectileSpawn + velOffset * 2f, velOffset * 0.7f, false, 30, Main.rand.NextFloat(0.4f, 0.65f), permafrost ? Color.HotPink : Main.rand.NextBool(3) ? Color.Lerp(Color.Red, Color.Magenta, 0.3f) : Color.Red);
                            ...
                            else if ((randomShot == 1 && canFireSplitingFireball) ||  fireFireblastFirst) // Firing gigablast while hovering above pre laugh
                            {
                                ...
                                {
                                    Vector2 velOffset = NPC.DirectionTo(player.Center).RotatedByRandom(0.6) * Main.rand.NextFloat(5f, 13f);
                                    GlowOrbParticle spark2 = new GlowOrbParticle(projectileSpawn + velOffset * 2f, velOffset * 0.8f, false, 30, Main.rand.NextFloat(0.4f, 0.65f), permafrost ? Color.HotPink : Main.rand.NextBool(3) ? Color.Lerp(Color.Red, Color.Magenta, 0.3f) : Color.Red);
                                    GeneralParticleHandler.SpawnParticle(spark2);
                                }
                            ...
                            else if (!fireFireblastFirst)
                            {
                                ...
                                for (int j = 0; j < numProj; j++)
                                {
                                    for (int i = 0; i < 6; i++) // Spread dust for pre laugh floating
                                    {
                                        ...
                                        GlowOrbParticle orb = new GlowOrbParticle(projectileSpawn, dustVel, false, 15, Main.rand.NextFloat(0.65f, 0.9f), permafrost ? Color.Magenta : Main.rand.NextBool() ? Color.Red : Color.Lerp(Color.Red, Color.Magenta, 0.3f));
                                        GeneralParticleHandler.SpawnParticle(orb);
                                    }
                    ...
                    else if (NPC.ai[1] == 3f)
                    {
                        ...
                        else
                        {
                            ...
                            if (NPC.ai[3] >= 20f)
                            {
                                ...
                                {
                                    Vector2 velOffset = NPC.DirectionTo(player.Center).RotatedByRandom(0.6) * Main.rand.NextFloat(5f, 13f);
                                    PointParticle spark2 = new PointParticle(handPosition + velOffset * 2f, velOffset * 1.5f, false, 18, Main.rand.NextFloat(0.4f, 0.65f), permafrost ? Color.HotPink : Main.rand.NextBool(3) ? Color.Lerp(Color.Red, Color.Magenta, 0.3f) : Color.Red);
                                    GeneralParticleHandler.SpawnParticle(spark2);
                                }
                        ...
                        {
                            GlowOrbParticle orb = new GlowOrbParticle(handPosition, new Vector2(0, -6).RotatedByRandom(0.4) * Main.rand.NextFloat(0.8f, 1.4f), false, 15, Main.rand.NextFloat(0.85f, 1.2f), permafrost ? Color.Magenta : Main.rand.NextBool() ? Color.Red : Color.Lerp(Color.Red, Color.Magenta, 0.3f), true, true);
                            GeneralParticleHandler.SpawnParticle(orb);
                        }
                    ...
                    else if (NPC.ai[1] == 4f)
                    {
                        ...
                        if (NPC.localAI[1] > shootRate) // Firing spam projectiles post laugh
                        {
                            ...
                            {
                                Vector2 velOffset = NPC.DirectionTo(player.Center).RotatedByRandom(0.6) * Main.rand.NextFloat(5f, 13f);
                                GlowOrbParticle spark2 = new GlowOrbParticle(handPosition + velOffset * 2f, velOffset * 1.5f, false, 9, Main.rand.NextFloat(0.4f, 0.65f), permafrost ? Color.HotPink : Main.rand.NextBool(3) ? Color.Lerp(Color.Red, Color.Magenta, 0.3f) : Color.Red);
                                GeneralParticleHandler.SpawnParticle(spark2);
                            }
                            NPC.localAI[1] = 0f;
            ...
            else if (NPC.ai[0] == 1f || NPC.ai[0] == 2f)
            {
                ...
                if (NPC.ai[1] == 100f)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Vector2 sparkVel = new Vector2(20, 20).RotatedByRandom(100) * Main.rand.NextFloat(0.1f, 1.1f);
                            GlowOrbParticle orb = new GlowOrbParticle(NPC.Center + sparkVel * 2, sparkVel, false, 120, Main.rand.NextFloat(1.55f, 2.75f), permafrost ? Color.Magenta : Color.Red, true, true);
                            GeneralParticleHandler.SpawnParticle(orb);
                        }
                        Particle pulse = new DirectionalPulseRing(NPC.Center, Vector2.Zero, permafrost ? Color.Magenta : Color.Red, new Vector2(2f, 2f), 0, 0f, 1.1f, 25);
                        GeneralParticleHandler.SpawnParticle(pulse);
                ...
                else
                {
                    if (NPC.ai[1] == 0f)
                    {
                        ...
                        if (NPC.localAI[1] > 60f)
                        {
                            ...
                            if (randomShot == 0 && canFireSplitingFireball && !fireFireblastFirst) // fireblast while floating post laugh
                            {
                                ...
                                Particle pulse = new DirectionalPulseRing(NPC.Center, projectileVelocity * 1.2f, permafrost ? Color.Pink : Color.Red, new Vector2(0.5f, 1f), projectileVelocity.ToRotation(), 0.92f, 0f, 55);
                                GeneralParticleHandler.SpawnParticle(pulse);
                                Particle pulse2 = new DirectionalPulseRing(NPC.Center, projectileVelocity * 1f, permafrost ? Color.Pink : Color.Magenta, new Vector2(0.5f, 1f), projectileVelocity.ToRotation(), 0.95f, 0.4f, 55);
                                GeneralParticleHandler.SpawnParticle(pulse2);
                            ...
                            else if ((randomShot == 1 && canFireSplitingFireball) ||  fireFireblastFirst) // Gigablast while floating post laugh
                            {
                                ...
                                Particle pulse = new DirectionalPulseRing(NPC.Center, projectileVelocity * 1.2f, permafrost ? Color.Pink : Color.Red, new Vector2(0.5f, 1f), projectileVelocity.ToRotation(), 0.95f, 0f, 55);
                                GeneralParticleHandler.SpawnParticle(pulse);
                                Particle pulse2 = new DirectionalPulseRing(NPC.Center, projectileVelocity * 1f, permafrost ? Color.Pink : Color.Magenta, new Vector2(0.5f, 1f), projectileVelocity.ToRotation(), 0.98f, 0.4f, 55);
                                GeneralParticleHandler.SpawnParticle(pulse2);
                            ...
                            else if (!fireFireblastFirst)
                            {
                                ...
                                for (int j = 0; j < numProj; j++)
                                {
                                    ...
                                    {
                                        Vector2 dustVel = (projectileVelocity * 2).RotatedByRandom(0.9) * (Main.rand.NextFloat(0.5f, 1.9f));
                                        GlowOrbParticle orb = new GlowOrbParticle(projectileSpawn, dustVel, false, 15, Main.rand.NextFloat(0.75f, 1f), permafrost ? Color.Magenta : Main.rand.NextBool() ? Color.Red : Color.Lerp(Color.Red, Color.Magenta, 0.3f), true, true);
                                        GeneralParticleHandler.SpawnParticle(orb);
                                    }
                    ...
                    else if (NPC.ai[1] == 3f)
                    {
                        ...
                        else
                        {
                            ...
                            if (NPC.ai[3] >= 24f)
                            {
                                ...
                                {
                                    Vector2 velOffset = NPC.DirectionTo(player.Center).RotatedByRandom(0.6) * Main.rand.NextFloat(5f, 13f);
                                    PointParticle spark2 = new PointParticle(handPosition + velOffset * 2f, velOffset * 1.5f, false, 9, Main.rand.NextFloat(0.5f, 0.75f), permafrost ? Color.HotPink : Main.rand.NextBool(3) ? Color.Lerp(Color.Red, Color.Magenta, 0.3f) : Color.Red);
                                    GeneralParticleHandler.SpawnParticle(spark2);
                                }
                        ...
                        {
                            GlowOrbParticle orb = new GlowOrbParticle(handPosition, new Vector2(0, -6).RotatedByRandom(0.4) * Main.rand.NextFloat(0.8f, 1.4f), false, 15, Main.rand.NextFloat(0.95f, 1.45f), permafrost ? Color.Magenta : Main.rand.NextBool() ? Color.Red : Color.Lerp(Color.Red, Color.Magenta, 0.5f), true, true);
                            GeneralParticleHandler.SpawnParticle(orb);
                        }
                    ...
                    else if (NPC.ai[1] == 4f)
                    {
                        ...
                        if (NPC.localAI[1] > shootRate) // small gigablast post laugh
                        {
                            ...
                            Particle pulse = new DirectionalPulseRing(NPC.Center, projectileVelocity * 9, permafrost ? Color.Pink : Color.Red, new Vector2(0.5f, 1f), projectileVelocity.ToRotation(), 0.9f, 0f, 60);
                            GeneralParticleHandler.SpawnParticle(pulse);
                            Particle pulse2 = new DirectionalPulseRing(NPC.Center, projectileVelocity * 8, permafrost ? Color.Pink : Color.Magenta, new Vector2(0.5f, 1f), projectileVelocity.ToRotation(), 0.93f, 0.4f, 60);
                            GeneralParticleHandler.SpawnParticle(pulse2);
        ...
        public void DoHeartsSpawningCastAnimation(Player target, bool death)
        {
            ...
            foreach (Vector2 heartSpawnPosition in heartSpawnPositions)
            {
                ...
                castMagicDust.color = permafrost ? Color.Pink : Color.Red;
            ...
            if (attackCastDelay == 0)
            {
                ...
                foreach (Vector2 heartSpawnPosition in heartSpawnPositions)
                {
                    ...
                    // Spawn Dragonfollies if Permafrost exists.
                    for (int i = 0; i < 20; i++)
                    {
        ...
        public void DoBrothersSpawningCastAnimation(int bodyWidth, int bodyHeight)
        {
            ...
            if (attackCastDelay < brothersSpawnCastTime - 45f && attackCastDelay >= 60f)
            {
                ...
                GlowOrbParticle orb = new GlowOrbParticle(leftDustPosition, Vector2.Zero, false, 20, 2.8f - attackCastDelay * 0.01f, permafrost ? Color.Pink : Color.Red, true, true);
                GeneralParticleHandler.SpawnParticle(orb);
                GlowOrbParticle orb2 = new GlowOrbParticle(rightDustPosition, Vector2.Zero, false, 20, 2.8f - attackCastDelay * 0.01f, permafrost ? Color.Pink : Color.Red, true, true);
                GeneralParticleHandler.SpawnParticle(orb2);
            }
        ...
        public void DrawForcefield(SpriteBatch spriteBatch)
        {
            ...
            Color secondaryForcefieldColor = (permafrost ? Color.LightBlue : Color.Red) * 1.4f;
