...
namespace CalamityMod.NPCs.AcidRain
{
    public class CragmawMire : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (Time % 420f > 240f && Time % 420f < 300f)
            {
                float vectorRotationAngle = (Time % 420f - 240f) / 60f * MathHelper.Pi * 4f;
                for (int i = 0; i < 25; i++)
                {
                    ...
                    if (!float.IsNaN(y))
                    {
                        Vector2 velocity = new Vector2((float)Math.Cos(angle), y) * MathHelper.Lerp(2.4f, 4.2f, (Time - 240f) / 60f);
                        ...
                        dust.noGravity = true;
                        dust.scale = MathHelper.Lerp(1.2f, 2.5f, (Time - 240f) / 60f);
                    }
                }

                float length = MathHelper.Lerp(20f, 1200f, (Time % 420f - 240f) / 60f);
                float outwardness = 1f - (Time % 420f - 240f) / 60f;
            ...
            // Release a laser beam upward and create an explosion.
            if (Time % 420f == 300f)
            {
            ...
            // Release acid into the air.
            else if (Time % 420f > 300f && Time % 7f == 6f && CalamityWorld.downedPolterghast)
            {
                if (Main.netMode != NetmodeID.MultiplayerClient)
        ...
        public void DoPhase1AI()
		{
            // Become harder to damage to if the target is not in water.
            npc.Calamity().DR = Target.wet ? 0.3f : 0.5f;

            // Use a rock-like hit sound.
            npc.HitSound = SoundID.NPCHit42;

            if (Time % 420f < 240f)
            {
                float bubbleShootRate = CalamityWorld.downedPolterghast ? 24f : 50f;
                if (Main.netMode != NetmodeID.MultiplayerClient && Time % bubbleShootRate == bubbleShootRate - 1f)
                {
                    int damage = CalamityWorld.downedPolterghast ? 47 : 30;
                    Vector2 bubbleSpawnPosition = npc.Top + Vector2.UnitY * 5f;
                    Vector2 bubbleShootVelocity = -Vector2.UnitY.RotatedByRandom(0.25f) * 4f;
                    int bubble = Projectile.NewProjectile(bubbleSpawnPosition, bubbleShootVelocity, ModContent.ProjectileType<CragmawBubble>(), damage, 1f);

                    if (Main.projectile.IndexInRange(bubble))
                    {
                        Main.projectile[bubble].timeLeft = Main.rand.Next(120, 180);
                        Main.projectile[bubble].netUpdate = true;
                    }
                }

                // Summon enemies from time to time.
                if (Time % 50f == 49f && Main.rand.NextBool(4))
                {
                    ...
                    SummonEnemyNearSelf(possibleEnemies);
                }
            }
        }

        public void DoPhase2AI()
        {
            // Release shell gore the first frame this enemy is in the second phase.
            if (!HasMadeShellBreakGore)
            {
            ...
            // Have DR depend on the current mode.
            npc.Calamity().DR = CalamityWorld.revenge ? 0.125f : 0f;

            // And use the squishy slime hit sound.
            npc.HitSound = SoundID.NPCHit1;

            // Release a tether that attempts to grab the target and then release spikes upward a bit afterwards.
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                if (Time % 600f == 20f)
                ...
                if (Time % 600f == 240f)
                {
                    ...
                    for (int i = 0; i < 16; i++)
                    {
                        float shootDirectionAngle = MathHelper.TwoPi / 16f * i;
                        float offsetAngle = Main.rand.NextFloatDirection() * MathHelper.TwoPi / 27f;
                        shootDirectionAngle += offsetAngle;
                        Projectile.NewProjectile(npc.Center, shootDirectionAngle.ToRotationVector2() * 6f, ModContent.ProjectileType<CragmawSpike>(), damage, 4f);
                    }
                }
            }
        }

        public void SummonEnemyNearSelf(IList<int> possibleEnemies)
        {
			if (Main.netMode == NetmodeID.MultiplayerClient)
				return;
            ...
            // Ensure that the potential spawn position is actually in the world.
            ...
            // As well as an open tile wiht water.
            ...
            NPC.NewNPC((int)spawnPosition.X, (int)spawnPosition.Y, Main.rand.Next(possibleEnemies));
        }
