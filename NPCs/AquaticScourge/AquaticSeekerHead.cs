    ...
    public class AquaticSeekerHead : ModNPC
    {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                if (!TailSpawned && NPC.ai[0] == 0f)
                {
                    ...
                    for (int num36 = 0; num36 < maxLength; num36++)
                    {
                        ...
                        {
                            lol = NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), ModContent.NPCType<AquaticSeekerBody>(), NPC.whoAmI);
                        }
                        else
                        {
                            lol = NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), ModContent.NPCType<AquaticSeekerTail>(), NPC.whoAmI);
                        }
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            ...
            if (NPC.life <= 0)
            {
                ...
                {
                    Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, Mod.Find<ModGore>("AquaticSeekerHead").Type, 1f);
                }
            }
        }
