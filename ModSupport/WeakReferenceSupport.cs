	...
	internal class WeakReferenceSupport
	{
		...
		private static void AddCalamityBosses(Mod bossChecklist, Mod calamity)
		{
			...
			// Ravager
			{
				...
				List<int> loot = new List<int>() { ItemType<RavagerBag>(), ItemType<FleshyGeodeT1>(), ItemType<FleshyGeodeT2>(), ItemType<Bloodstone>(), ItemType<VerstaltiteBar>(), ItemType<DraedonBar>(), ItemType<CruptixBar>(), ItemType<CoreofCinder>(), ItemType<CoreofEleum>(), ItemType<CoreofChaos>(), ItemType<BarofLife>(), ItemType<CoreofCalamity>(), ItemType<UltimusCleaver>(), ItemType<RealmRavager>(), ItemType<Hematemesis>(), ItemType<SpikecragStaff>(), ItemType<CraniumSmasher>(), ItemType<BloodPact>(), ItemType<FleshTotem>(), ItemType<BloodflareCore>(), ItemType<InfernalBlood>(), ItemID.GreaterHealingPotion };
