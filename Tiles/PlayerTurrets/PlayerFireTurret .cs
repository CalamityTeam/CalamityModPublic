...
{
    public class PlayerFireTurret : ModTile
    {
        ...
        {
            Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 32, 32, ModContent.ItemType<FireTurret>(), 1);

            Tile t = Main.tile[i, j];
            int left = i - t.TileFrameX % (Width * SheetSquare) / SheetSquare;
            int top = j - t.TileFrameY % (Height * SheetSquare) / SheetSquare;

            TEPlayerFireTurret te = CalamityUtils.FindTileEntity<TEPlayerFireTurret>(i, j, Width, Height, SheetSquare);
            te?.Kill(left, top);
        }
