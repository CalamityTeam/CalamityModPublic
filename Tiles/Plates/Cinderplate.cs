...
namespace CalamityMod.Tiles.Plates
{
    public class Cinderplate : ModTile
    {
        ...
        public override void SetStaticDefaults()
        {
            if (!Main.dedServ)
            {
                PulseTexture = ModContent.Request<Texture2D>("CalamityMod/Tiles/Plates/CinderplatePulse");
                PulseColors = new Color[PulseTexture.Width];
                PulseTexture.GetData(PulseColors);
                GlowTexture = ModContent.Request<Texture2D>("CalamityMod/Tiles/Plates/CinderplateGlow");
            }
