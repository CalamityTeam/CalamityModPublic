...
namespace CalamityMod.Items.CalamityCustomThrowingDamage
{
    public class BlazingStar : CalamityDamageItem
    {
        ...
        public override void SafeSetDefaults()
        {
            ...
            item.shoot = mod.ProjectileType("BlazingStarProj");
        }
            ...
            if (player.Calamity().StealthStrikeAvailable())
            {
                if (item.stack != 1)
                {
                    for (int i = 0; i < item.stack; i++)
                    {
                        ...
                        int projectileIndex2 = Projectile.NewProjectile(position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y, i == 1 ? type : mod.ProjectileType("BlazingStarProj"), damage, knockBack, player.whoAmI, 0f, 0f);
