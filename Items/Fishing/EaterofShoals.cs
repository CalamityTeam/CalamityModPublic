...
namespace CalamityMod.Items.Fishing
{
    public class EaterofShoals : ModItem
    {
        public override void SetStaticDefaults()
        {
            ...
            Tooltip.SetDefault("Fires ten lines at once. Line never snaps.\n" +
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            for (int index = 0; index < 10; ++index)
            {
                ...
                int linecolor = Projectile.NewProjectile(position.X, position.Y, SpeedX, SpeedY, ModContent.ProjectileType<EaterofShoalsBobber>(), 0, 0f, player.whoAmI, 0.0f, 0.0f);
				if (Main.rand.NextBool(2)) //randomizing line color
				{
					Main.projectile[linecolor].Calamity().stealthStrike = true; //probably should use a different variable
				}
            }
            return false;
        }
