...
namespace CalamityMod.Items
{
    public class AncientShiv : ModItem
    {
        ...
        public override void OnHitNPC(Player player, NPC target, int damage, float knockback, bool crit)
        {
            if (target.life <= 0)
            {
                Projectile.NewProjectile(target.Center.X, target.Center.Y, 0f, 0f, ModContent.ProjectileType<Projectiles.BlueAura>(), (int)((float)item.damage * player.meleeDamage), knockback, Main.myPlayer);
            }
        }
    }
}
