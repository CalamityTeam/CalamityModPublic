		...
		public override bool Shoot(Player player, ref Microsoft.Xna.Framework.Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
	    {
			if(Collision.SolidCollision(position, player.width, player.height) && player.velocity.X != 0)
			{
				for(int i = 0; i < 5; i++)
				{
					...
					Projectile.NewProjectile((player.Center.X + posX), (player.Center.Y + 20), velocityX, (float)Main.rand.Next(-7, -3), mod.ProjectileType("UltimusCleaverDust"), (int)((float)item.damage * 0.4), 0f, Main.myPlayer);
				}
			}
			return false;
		}
