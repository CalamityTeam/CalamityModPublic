			...
			if (target.life <= 0)
			{
	    		for (i = 0; i < 1; i++ )
				{
					...
					float randomSpeedY = (float)Main.rand.Next(3, 5);
				   	...
				   	int projectile1 = Projectile.NewProjectile(target.Center.X, target.Center.Y, (float)( Math.Sin(offsetAngle) * 5f ), (float)( Math.Cos(offsetAngle) * 5f ), mod.ProjectileType("BurntSienna"), damage, knockback, Main.myPlayer);
				    int projectile2 = Projectile.NewProjectile(target.Center.X, target.Center.Y, (float)( -Math.Sin(offsetAngle) * 5f ), (float)( -Math.Cos(offsetAngle) * 5f ), mod.ProjectileType("BurntSienna"), damage, knockback, Main.myPlayer);
					int projectile3 = Projectile.NewProjectile(target.Center.X, target.Center.Y, (float)( -Math.Sin(offsetAngle) * 5f ), (float)( -Math.Cos(offsetAngle) * 5f ), mod.ProjectileType("BurntSienna"), damage, knockback, Main.myPlayer);
