        ...
        private void OnHitEffects(Player player, bool health, Vector2 targetPos, int targetWidth, int targetHeight, float kBack)
        {
            if (health)
            {
                ...
                for (int i = 0; i < bloodAmt; i++)
                {
                    ...
                    Projectile.NewProjectile(source, targetPos, velocity, ModContent.ProjectileType<Blood>(), (int)(Item.damage * (player.GetDamage<GenericDamageClass>().Additive + player.GetDamage(DamageClass.Melee) - 1f)), kBack, player.whoAmI, 0f, 0f);
                }
            }
        }
    }
