...
namespace CalamityMod.Items.Weapons
{
    class Onyxia : ModItem
    {
        ...
        public override bool Shoot(Player player, ref Microsoft.Xna.Framework.Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            ...
            Projectile.NewProjectile(position.X, position.Y, shardVelocityX, shardVelocityY, 661, shardDamage, shardKB, player.whoAmI, 0.0f, 0.0f);
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                Projectile.NewProjectile(position.X, position.Y, left.X, left.Y, type, damage, knockBack, player.whoAmI, 0.0f, 0.0f);
                Projectile.NewProjectile(position.X, position.Y, right.X, right.Y, type, damage, knockBack, player.whoAmI, 0.0f, 0.0f);
            }
            return false;
        }
