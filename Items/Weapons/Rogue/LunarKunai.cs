        ...
        public override bool Shoot(Player player, ref Microsoft.Xna.Framework.Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
			...
			if (player.Calamity().StealthStrikeAvailable())
			{
				for (int i = 0; i < 12; i++)
				{
					...
					float x4 = vector2.X;
					float y4 = vector2.Y;
					int stealth = Projectile.NewProjectile(x4, y4, num148, num149, ModContent.ProjectileType<LunarKunaiProj>(), (int)((double)item.damage * 0.75), knockBack, player.whoAmI, 0f, 0f);
