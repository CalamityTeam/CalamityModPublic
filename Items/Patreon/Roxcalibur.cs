...
namespace CalamityMod.Items.Patreon
{
	public class Roxcalibur : ModItem
	{
		...
		public override void SetDefaults()
		{
			...
			item.shoot = mod.ProjectileType("Rox1");
			item.shootSpeed = 10f;
		...
		public override void UpdateInventory(Player player)
		{
			...
			if (RoxAlt && player.ownedProjectileCounts[mod.ProjectileType("RoxSlam")] <= 0)
			{
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			if (item.useStyle == 4)
			{
				...
				Projectile.NewProjectile(positionx, positiony, player.velocity.X, player.velocity.Y, mod.ProjectileType("RoxSlam"), 0, 0, player.whoAmI, cooldown);
			...
			else
			{
				for (int i = 0; i < 3; i++)
				{
					...
					Projectile.NewProjectile(position.X, position.Y, pertubedspeed.X, pertubedspeed.Y, mod.ProjectileType("Rox1"), (int)((double)damage * 0.5), 1f, player.whoAmI, Main.rand.Next(3));
				}
				RoxCanAlt = 0;
