...
namespace CalamityMod.Cooldowns
{
	public class CooldownRackUI
	{
		...
		public static bool CompactIcons => Main.LocalPlayer.Calamity().Cooldowns.FindAll(cooldown => cooldown.ShouldDisplay).Count > MaxLargeIcons || CalamityConfig.Instance.CooldownDisplay == 1;
		...
		public static void Draw(SpriteBatch spriteBatch)
		{
			...
			List<Cooldown> cooldownsToDraw = Main.LocalPlayer.Calamity().Cooldowns.FindAll(cooldown => cooldown.ShouldDisplay);
			...
			foreach (Cooldown cd in cooldownsToDraw)
			{
				...
				{
					mouseHover = cd.DisplayName;
					iconOpacity = MathHelper.Clamp((float)Math.Sin(Main.GlobalTime % MathHelper.Pi) * 2f, 0, 1) * 0.1f + 0.9f;
				}

				if ((cd.UseCustomExpandedDraw && !CompactIcons) || (cd.UseCustomCompactDraw && CompactIcons))
				{
					if (CompactIcons)
						cd.DrawCompact(spriteBatch, displayPosition, iconOpacity, uiScale);
					else
						cd.DrawExpanded(spriteBatch, displayPosition, iconOpacity, uiScale);
				}
		...
		public static void DrawIndividualIcon(Cooldown cooldown, SpriteBatch spriteBatch, Vector2 position, float opacity, float scale)
		{
			Texture2D sprite = ModContent.GetTexture(cooldown.Texture);
			Texture2D outline = ModContent.GetTexture(cooldown.OutlineTexture);
			...
			if (CompactIcons)
			{
				Texture2D overlay = ModContent.GetTexture(cooldown.OverlayTexture);
