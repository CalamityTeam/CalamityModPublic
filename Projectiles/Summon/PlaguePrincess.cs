        ...
        public override void AI()
        {
            ...
            if (player.HasMinionAttackTargetNPC)
            {
                ...
                if (npc.CanBeChasedBy(projectile, false))
                {
                    ...
                    if (((Vector2.Distance(projectile.Center, targetLocation) > num646 && num646 < num633) || !targetFound) && Collision.CanHitLine(projectile.position, projectile.width, projectile.height, npc.position, npc.width, npc.height))
                    {
                        num633 = num646;
            ...
            else
            {
                for (int num645 = 0; num645 < Main.maxNPCs; num645++)
                {
                    ...
                    if (nPC2.CanBeChasedBy(projectile, false))
                    {
                        ...
                        if (((Vector2.Distance(projectile.Center, targetLocation) > num646 && num646 < num633) || !targetFound) && Collision.CanHitLine(projectile.position, projectile.width, projectile.height, nPC2.position, nPC2.width, nPC2.height))
                        {
                            num633 = num646;
