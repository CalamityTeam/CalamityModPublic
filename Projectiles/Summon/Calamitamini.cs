...
namespace CalamityMod.Projectiles.Summon
{
    public class Calamitamini : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			float num635 = 2400f;
			float num636 = 150f;
			bool flag64 = projectile.type == mod.ProjectileType("Calamitamini");
			Player player = Main.player[projectile.owner];
			CalamityPlayer modPlayer = player.GetModPlayer<CalamityPlayer>(mod);
			player.AddBuff(mod.BuffType("CalamitasEyes"), 3600);
			if (flag64)
			{
				if (player.dead)
				{
			...
			{
				bool flag23 = (Main.projectile[num638].type == mod.ProjectileType("Calamitamini"));
