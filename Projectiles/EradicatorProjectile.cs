			...
			if (flag14)
			{
				...
				if (projectile.localAI[0] > 8f)
				{
					...
					if (Main.rand.Next(2) == 0)
					{
						...
						{
							int laser = Projectile.NewProjectile(value10.X, value10.Y, num438, num439, mod.ProjectileType("NebulaShot"), projectile.damage, projectile.knockBack, projectile.owner, 0f, 0f);
                            Main.projectile[laser].ranged = false;
                            Main.projectile[laser].thrown = true;
                        }
					}
					return;
				}
