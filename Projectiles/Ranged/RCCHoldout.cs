...
{
    public class RCCHoldout : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            else
            {
                ...
                if (CurrentChargingFrames >= FramesToLoadBolt && LoadedBolts < RCCAnihilator.MaxBolts)
                {
                    ...
                    if (LoadedBolts >= RCCAnihilator.MaxBolts)
        ...
        public void ShootProjectiles(Vector2 tipPosition, float projectileRotation)
        {
            ...
            int BoltDamage = (int)(heldItem.damage * Owner.RangedDamage() * (LoadedBolts + 1) / (float)(RCCAnihilator.MaxBolts + 1));
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            if (LoadedBolts == RCCAnihilator.MaxBolts) //If the bow is fully loaded, shave off the part where you draw the arrow that's charging currently
                loops = LoadedBolts;
            ...
            for (int i = 0; i < loops; i++)
            {
                ...
                else if (LoadedBolts == RCCAnihilator.MaxBolts)
                {
                    float increment = angularSpread * (LoadedBolts - 1) / 2;
                ...
                if (i == LoadedBolts - 1 || LoadedBolts == RCCAnihilator.MaxBolts) //If the arrow we are looking at is the one that just got loaded, OR all arrows got loaded, we apply some flashiness
                {
                    ...
                    GameShaders.Misc["CalamityMod:BasicTint"].UseColor(Main.hslToRgb(1f - (LoadedBolts / RCCAnihilator.MaxBolts), 0.8f, 0.85f));
                ...
                if (i == LoadedBolts - 1 || LoadedBolts == RCCAnihilator.MaxBolts) //Don't forget to exit the shader region
