...
namespace CalamityMod.Projectiles.Boss
{
    public class HolyBlast : ModProjectile
    {
        ...
        {
            Texture2D texture = (Main.dayTime && !CalamityWorld.malice) ? ModContent.Request<Texture2D>(Texture).Value : ModContent.Request<Texture2D>("CalamityMod/Projectiles/Boss/HolyBlastNight").Value;
            int num214 = texture.Value.Height / Main.projFrames[Projectile.type];
        ...
        public override void Kill(int timeLeft)
        {
            if (Projectile.owner == Main.myPlayer)
            {
                ...
                for (int i = 0; i < totalProjectiles; i++)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), shootFromVector.X, shootFromVector.Y, (float)(Math.Sin(offsetAngle) * velocity), (float)(Math.Cos(offsetAngle) * velocity), type, damage, 0f, Main.myPlayer, 0f, 0f);
                    Projectile.NewProjectile(Projectile.GetProjectileSource_FromThis(), shootFromVector.X, shootFromVector.Y, (float)(-Math.Sin(offsetAngle) * velocity), (float)(-Math.Cos(offsetAngle) * velocity), type, damage, 0f, Main.myPlayer, 0f, 0f);
                }
            }
