    ...
    public class SpatialSpear3 : ModProjectile, ILocalizedModType
    {
            ...
            if (Projectile.localAI[0] >= 30f)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    for (int i = 0; i < numProj + 1; i++)
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, perturbedSpeed, ModContent.ProjectileType<SpatialSpear4>(), (int)(Projectile.damage * 0.5), Projectile.knockBack * 0.5f, Projectile.owner, 0f, 0f);
                    }
                }
            }
        }
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 4; i < 12; i++)
            {
                ...
                int spatial = Dust.NewDust(new Vector2(Projectile.oldPosition.X - projOldX, Projectile.oldPosition.Y - projOldY), 8, 8, 73, Projectile.oldVelocity.X, Projectile.oldVelocity.Y, 100, default, 1.8f);
                Main.dust[spatial].noGravity = true;
                Dust dust = Main.dust[spatial];
                dust.velocity *= 0.5f;
                spatial = Dust.NewDust(new Vector2(Projectile.oldPosition.X - projOldX, Projectile.oldPosition.Y - projOldY), 8, 8, 73, Projectile.oldVelocity.X, Projectile.oldVelocity.Y, 100, default, 1.4f);
