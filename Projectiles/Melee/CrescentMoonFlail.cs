...
{
    public class CrescentMoonFlail : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.alpha == 0)
            {
                ...
                {
                    Dust dust = Main.dust[Dust.NewDust(projectile.position + projectile.velocity * 2f, projectile.width, projectile.height, 176, 0f, 0f, 100, default, 2f)];
                    dust.noGravity = true;
                    dust.velocity *= 2f;
                ...
                while ((float)num53 < num52)
                {
                    if (Main.rand.NextBool(4))
                    {
                        ...
                        Dust dust2 = Main.dust[Dust.NewDust(position, projectile.width, projectile.height, 20, 0f, 0f, 100, default, 1f)];
            ...
            if (moonCounter <= 0 && projectile.owner == Main.myPlayer)
            {
				...
				int moon = Projectile.NewProjectile(newCenter.X, newCenter.Y, 0f, 0f, ModContent.ProjectileType<CrescentMoonProj>(), (int)((double)projectile.damage * 0.4), 0f, projectile.owner, 0f, 0f);
				moonCounter = 6;
			}
        }
