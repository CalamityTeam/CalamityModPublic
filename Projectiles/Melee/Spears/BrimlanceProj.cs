    ...
    public class BrimlanceProj : BaseSpearProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            if (target.life <= 0)
            {
                ...
                {
                    Projectile.NewProjectile(target.Center, Vector2.Zero, ModContent.ProjectileType<BrimlanceHellfireExplosion>(), (int)(projectile.damage * 0.75), knockback, Main.myPlayer);
                    for (int i = 0; i < flameCount; i++)
                    {
                        Vector2 fireVelocity = new Vector2(0f, Main.rand.NextFloat(7f, 10f)).RotatedByRandom(MathHelper.TwoPi);
                        Projectile.NewProjectile(target.Center, fireVelocity, ModContent.ProjectileType<BrimlanceStandingFire>(), (int)(projectile.damage * 0.5), 0f, projectile.owner, 0f, 0f);
                    }
                }
            }
        }
