        ...
        public override void AI()
        {
            ...
            if (projectile.ai[0] % 20 == 0f)
            {
                for (int l = 0; l < 24; l++)
                {
                    ...
                    spawnPositionAdditive += -Vector2.UnitY.RotatedBy((double)((float)l * MathHelper.TwoPi / 24f), default) * new Vector2(8f, 16f) * projectile.scale;
                    spawnPositionAdditive = spawnPositionAdditive.RotatedBy((double)(projectile.rotation), default);
                    int dustIndex = Dust.NewDust(projectile.Center, 0, 0, 218, 0f, 0f, 0, default, 1.5f);
