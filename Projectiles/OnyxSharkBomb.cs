    ...
    public class OnyxSharkBomb : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
	    	...
	    	for (i = 0; i < 2; i++ )
	    	{
	        	...
	        	Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, (float)( Math.Sin(offsetAngle) * 5f ), (float)( Math.Cos(offsetAngle) * 5f ), mod.ProjectileType("SandyWaifuShark"), (int)((double)projectile.damage * 0.4f), projectile.knockBack, projectile.owner, 0f, 0f);
	        	Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, (float)( -Math.Sin(offsetAngle) * 5f ), (float)( -Math.Cos(offsetAngle) * 5f ), mod.ProjectileType("SandyWaifuShark"), (int)((double)projectile.damage * 0.4f), projectile.knockBack, projectile.owner, 0f, 0f);
	    	}
        }
    }
