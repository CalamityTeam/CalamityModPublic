        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
			...
			if (projectile.owner == Main.myPlayer)
			{
				for (i = 0; i < 4; i++ )
				{
					...
					Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, (float)( Math.Sin(offsetAngle) * 5f ), (float)( Math.Cos(offsetAngle) * 5f ), mod.ProjectileType("PlasmaRay2"), (int)((double)projectile.damage * 1.75f), projectile.knockBack, projectile.owner, 0f, 0f);
					Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, (float)( -Math.Sin(offsetAngle) * 5f ), (float)( -Math.Cos(offsetAngle) * 5f ), mod.ProjectileType("PlasmaRay2"), (int)((double)projectile.damage * 1.75f), projectile.knockBack, projectile.owner, 0f, 0f);
				}
			}
