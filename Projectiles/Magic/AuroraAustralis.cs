        ...
        public override void AI()
        {
            ...
            if (projectile.timeLeft % 10 == 0 && Main.myPlayer == projectile.owner) //spawn stars every 10 ticks
            {
                if (Main.player[projectile.owner].ownedProjectileCounts[ModContent.ProjectileType<AstralStarMagic>()] < 30)
                {
					...
					float xStart = projectile.Center.X + projectile.velocity.X;
					float yStart = projectile.Center.Y - Main.rand.Next(500, 800);
					...
					float starSpeed = Main.rand.NextFloat(10f, 20f);
					travelDist = starSpeed / travelDist;
					velocity.X *= travelDist;
					velocity.Y *= travelDist;
					int star = Projectile.NewProjectile(startPos, velocity, ModContent.ProjectileType<AstralStarMagic>(), (int)(projectile.damage * dmgKBMult), projectile.knockBack * dmgKBMult, projectile.owner, 0f, 0f);
					Main.projectile[star].timeLeft = 120;
                }
            }
