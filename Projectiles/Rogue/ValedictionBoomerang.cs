        ...
        private void OnHitEffects()
        {
            ...
            if (Projectile.owner == Main.myPlayer && Projectile.numHits < 1 && Projectile.Calamity().stealthStrike)
            {
                ...
                for (int typhoonCount = 0; typhoonCount < typhoonAmt; typhoonCount++)
                {
                    ...
                    Projectile typhoon = Projectile.NewProjectileDirect(Projectile.GetProjectileSource_FromThis(), Projectile.Center, velocity, ModContent.ProjectileType<NuclearFuryProjectile>(), typhoonDamage, 0f, Projectile.owner);
