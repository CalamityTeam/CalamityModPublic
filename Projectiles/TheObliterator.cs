        ...
        public override void AI()
        {
			...
			if (flag14)
			{
				...
				if (projectile.localAI[0] > 16f)
				{
					...
					if (projChoice == 0)
					{
						projChoice = mod.ProjectileType("NebulaShot");
					}
					else
					{
						projChoice = mod.ProjectileType("IceBeam");
					}
					if (projectile.owner == Main.myPlayer)
					{
						int shoot = Projectile.NewProjectile(value10.X, value10.Y, num438, num439, projChoice, (int)((double)projectile.damage * 0.75f), projectile.knockBack, projectile.owner, 0f, 0f);
						Main.projectile[shoot].ranged = false;
						Main.projectile[shoot].melee = true;
					}
					return;
				}
			}
